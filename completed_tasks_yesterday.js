// This Remember the Milk MilkScript outputs the tasks that you completed *yesterday* in 2 formats:
...
//
// Written by Khalid J Hosein, Dec 2023.
// https://www.khalidjhosein.net


// Get the list of tasks that were completed Today:
const tasks = rtm.getTasks('completed:Yesterday');
...
const filename = yesterdaysDateNumeric() + " - Completed Tasks (via RTM).md.txt"
...
function listTasks(tasks, withDetails = 0) {
    ...
    if(withDetails == 1) {
        ...
        header = "# Completed yesteday, " + yesterdaysDate() + "\n";
    }
...
function yesterdaysDateNumeric() {
    var date = new Date();
    date.setDate(date.getDate() - 1);
